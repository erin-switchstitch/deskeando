...
let calculateCurrentWeek = (n, calculateCurrentDayWeek, originalDate) => {
	...
	if (
		n != 6 + 7 * calculateCurrentDayWeek &&
		n != 7 + 7 * calculateCurrentDayWeek &&
		n != 0 &&
		n > 7
	) {
...
const ListCalender = (props) => {
	...
	const [allDates, setAllDates] = useState(
		calculateCurrentWeek(todaysNumber, nextButtonClickCount, originalDate)
	);
			...
			<div className="ListCalenderInnerWrapper">
				<div className="This-week">
					...
					{allDates.map((date, index) => {
						...
						if (
							!String(date[0]._d).includes("Sat") &&
							!String(date[0]._d).includes("Sun")
						) {
							return (
								<div key={index}>
									<label>
										<input
											...
											onClick={(e) =>
												setGlobalBookingInfo({
													...globalBookingInfo,
													date_booked: moment(date[0]._d).format("YYYY-MM-D"),
												})
											}
										/>
										<Moment format="dddd, MMMM Do, YYYY">{date[0]}</Moment>- (AM
										: <strong>{50 - morningSpacesTaken}</strong> Spaces) | (PM :
										<strong>{50 - afternoonSpacesTaken} </strong>Spaces)
									</label>
								</div>
							);
						}
				...
				<div className="listCalenderButtonContainer">
					<button
						onClick={() => {
							...
							let allDays = calculateCurrentWeek(
								todaysNumber,
								tempClickCounter
							);
							console.log(allDays);
							let daysInCurrentWeek = allDays.splice(
								allDays.length - 7,
								allDays.length
							);
						...
						className="btn btn-default listCalenderButtons"
						type="submit"
					>
						Previous
					</button>

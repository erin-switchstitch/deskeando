...
export default function BookingPage(props) {
	...
	const [selectedDateParent, setSelectedDateParent] = useState(
		...
		if (bool == true) {
			return (
				<BookingSVG
					...
					selectedDateParent={selectedDateParent}
					setSelectedDateParent={(data) => setSelectedDateParent(data)}
			...
			.then((response) => {
				if (response.status == 200) {
					setGlobalBookingInfo({
						...globalBookingInfo,
						other_id: response.data.id,
						other_first_name: response.data.first_name,
						other_last_name: response.data.last_name,
					});
	...
	return (
		<div className="BookingPageOuterWrapper">
			<div className="BookingPageInnerHeaderWrapper">
				...
				<div className="BookingForPostClick">
					<div
						...
						className="BookingForPostClickInnerWrapper"
					>
						...
						<div
							style={{ display: `${person === "Myself" ? "none" : "flex"}` }}
							className="BookingForInputWrapper"
						>
							...
							<input
								placeholder="Enter the person's registered email"
								type="text"
								value={otherPersonsEmail}
								onChange={(e) => {
									setGlobalBookingInfo({
										...globalBookingInfo,
										other_email: e.target.value,
									});
			...
			<div className="BookingPageInnerMainWrapper">
				...
				(globalBookingInfo.am != false || globalBookingInfo.pm != false) ? (
					width < 800 ? (
						<DeskList
							globalBookingInfo={globalBookingInfo}
							setGlobalBookingInfo={(data) => setGlobalBookingInfo(data)}
				...
				{globalBookingInfo.desk_id != "" &&
				globalBookingInfo.date_booked != "" &&
				(globalBookingInfo.am != false || globalBookingInfo.pm != false) ? (
					<DeskListBooker
						otherPersonsName={{ thirdPartyFirstName, thirdPartyLastName }}
						globalBookingInfo={globalBookingInfo}
						setGlobalBookingInfo={(data) => setGlobalBookingInfo(data)}
						selectedDateParent={selectedDateParent}
						setSelectedDateParent={(data) => setSelectedDateParent(data)}
